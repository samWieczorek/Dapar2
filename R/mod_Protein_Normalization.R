...
mod_Protein_Normalization_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    mod_plots_density_server(id = "densityPlot_Norm",
                             obj = reactive({obj}),
   ...
   # callModule(module_Not_a_numeric,"test_spanLOESS", reactive({rv$widgets$normalization$spanLOESS}))
    ...
    mod_popover_for_help_server("modulePopover_normQuanti",
                                data = list(title= 'Normalization quantile',
  ...
  rv.norm$selectProt <- mod_plots_tracking_server('master_tracking',
    ...
    observeEvent(input$norm_method, {rv.widgets$norm_method <- input$norm_method})
    observeEvent(input$norm_type, {rv.widgets$norm_type <- input$norm_type})
    observeEvent(input$norm_spanLOESS, {rv.widgets$norm_spanLOESS <- input$norm_spanLOESS})
    observeEvent(input$norm_quantile, {rv.widgets$norm_quantile <- input$norm_quantile})
    observeEvent(input$norm_varReduction, {rv.widgets$norm_varReduction <- input$norm_varReduction})
    observeEvent(input$norm_sync, {rv.widgets$norm_sync <- input$norm_sync
    })
    ...
    output$norm_scaling_ui <- renderUI({
      ...
      if (rv.widgets$norm_method == "MeanCentering"){
        ...
        if (rv$steps.enabled['Normalization'])
          checkboxInput(ns("norm_varReduction"), "Include variance reduction",  
    ...
    observeEvent(rv.widgets$norm_method,{
      ...
      # shinyjs::toggle("perform.normalization", condition = rv.widgets$norm_method != "None")
      # shinyjs::toggle("spanLOESS", condition = rv.widgets$norm_method == "LOESS")
      # 
      # shinyjs::toggle("normalization.type", 
      #                 condition=( rv.widgets$norm_method %in% c("QuantileCentering", "MeanCentering", "SumByColumns", "LOESS", "vsn")))
      # 
    ...
    output$Normalization <- renderUI({
      ...
      wellPanel(id = ns('toto'),
                tagList(
                  ...
                  tags$hr(),
                  fluidRow(
                    column(width=4, moduleDensityplotUI(ns("densityPlot_Norm"))),
                    column(width=4,
