...
match.qMetadata <- function(df, pattern, level) {
    ...
    if (!(pattern %in% qMetadata.def(level)$node)) {
        stop(paste0(
            "'pattern' is not correct. Availablevalues are: ",
            paste0(qMetadata.def(level)$node, collapse = " ")
        ))
    }

    ll.res <- lapply(
        search.qMetadata.tags(pattern = pattern, level),
        function(x) {
            as.data.frame(df) == x
        }
    )
...
#'
setMethod(
    "UpdateqMetadata", "SummarizedExperiment",
    function(object,
             from,
             to,
             ...) {
        ...
        ind <- match.qMetadata(
            metadata = qMetadata(object),
            pattern = na.type,
            level = level
        ) & !is.na(SummarizedExperiment::assay(object))
...
search.qMetadata.tags <- function(pattern, level, depth = "1") {
    ...
    tags <- NULL
    .ind <- which(qMetadata.def(level)$parent == pattern)
    tags <- switch(depth,
        "0" = {pattern},
        "1" = c(pattern, qMetadata.def(level)$node[.ind]),
        "*" = {
            if (length(qMetadata.def(level)$node[.ind]) == 0) {
                search.qMetadata.tags(pattern, level, "0")
            } else {
                c(pattern, 
                    unlist(lapply(
                        qMetadata.def(level)$node[.ind],
                        function(x) search.qMetadata.tags(x, level, depth)
                        )
                        )
                    )
            }
...
qMetadata_combine <- function(met, level) {
    ...
    ComputeNbTags <- function(tag) {
        sum(unlist(lapply(
            search.qMetadata.tags(tag, level),
