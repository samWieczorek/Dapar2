...
##'
##' @rdname QFeatures-filtering
setMethod("filterFeaturesOneSE",
          c("QFeatures", "AnnotationFilter"),
          function(object, filter, na.rm = FALSE, ...)
            filterFeaturesOneSEWithAnnotationFilter(object, filter, na.rm, ...))

##' @rdname QFeatures-filtering
setMethod("filterFeaturesOneSE",
          c("QFeatures", "formula"),
          function(object, filter, na.rm = FALSE, ...)
            filterFeaturesOneSEWithFormula(object, filter, na.rm, ...))

##' @importFrom BiocGenerics do.call
filterFeaturesOneSEWithAnnotationFilter <- function(object, filter, na.rm, ...) {
...
filterFeaturesOneSEWithFormula <- function(object, filter, na.rm, ...) {
  sel <- lapply(experiments(object),
                function(exp) {
                  x <- rowData(exp)
