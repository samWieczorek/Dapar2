...
#' @export
#' 
ComputeConnexComposants <- function(X.list=NULL){
  if( is.null(X.list)){
    warning("Cannot compute because the matrix X is NULL")
    return(NULL)
  }
  
  res <- list()
  if (!is.null(X.list$all)){
    res$all <- get.pep.prot.cc(X.list$all)
  }
  
  if (!is.null(X.list$onlyShared)){
    res$onlyShared <- get.pep.prot.cc(X.list$onlyShared)
  }
  if (!is.null(X.list$onlySpec)){
    res$onlySpec <- get.pep.prot.cc(X.list$onlySpec)
  }
...
#' @title Display a connex component.
#' 
#' @description Displays a connex component as a graph with the package 'visNetwork'.
#' 
